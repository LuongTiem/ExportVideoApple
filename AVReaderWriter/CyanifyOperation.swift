...
class CyanifyOperation : Operation {
    ...
    private func makeVideoWriterInputsForVideoReaderOutputs(videoReaderOutputs: [AVAssetReaderTrackOutput])  throws -> [ReaderOutputAndWriterInput] {
        ...
        for (videoReaderOutput, tempVideoReaderOutput) in videoReaderOutputsAndTempVideoReaderOutputs {
            ...
            //MARK: KHOI TAO ADAPTER
            let sourcePixelBufferAttributes = [kCVPixelBufferPixelFormatTypeKey as String : NSNumber(value: kCVPixelFormatType_32ARGB)]
            adaptor = AVAssetWriterInputPixelBufferAdaptor(assetWriterInput: videoWriterInput, sourcePixelBufferAttributes: sourcePixelBufferAttributes)
    ...
    private func transferVideoTracks(videoReaderOutputsAndWriterInputs: [ReaderOutputAndWriterInput], group : DispatchGroup) {
        ...
        for (videoReaderOutput, videoWriterInput) in videoReaderOutputsAndWriterInputs {
            ...
            let videoProcessor: (CMSampleBuffer) throws -> Void = { sampleBuffer in
                /* CMSampleBuffer là toàn bộ các frame hình ảnh có trong video
                 eg: video 7s ->> có 209 ảnh (sample buffer)
                 --> edit
                 */
                
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer),
                    let pixelBuffer: CVPixelBuffer = imageBuffer, CFGetTypeID(imageBuffer) == CVPixelBufferGetTypeID() {
                    //                                        let redComponentIndex = 3
...
//                    self.adaptor.append(CVPixelBuffer, withPresentationTime: <#T##CMTime#>)
    ...
    private func transferSamplesAsynchronouslyFromReaderOutput(_ readerOutput: AVAssetReaderOutput,
        ...
        writerInput.requestMediaDataWhenReady(on: queue) {
            ...
            while writerInput.isReadyForMoreMediaData && self.adaptor.assetWriterInput.isReadyForMoreMediaData {
                guard !self.isCancelled else {
                    isDone = true
                    break
                }
    ...
    //MARK: pixelBufferFromCGImage
    func pixcelBufferFromCGImage(image : CGImage , size : CGSize) -> CVPixelBuffer? {
        ...
        if let _pxbuffer = pxbuffer {
            ...
            let context = CGContext(data: pxdata,
                                    ...
                                    height: imageHeight,
                                    bitsPerComponent: 8,
                                    bytesPerRow: CVPixelBufferGetBytesPerRow(_pxbuffer), // co the thay doi neu width tang
                space: rgbColorSpace,
